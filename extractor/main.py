...
datalake_path = sys.argv[2]

SOURCE_FILE_PATH = f"{assets_path}/vendas-combustiveis-m3.xls"
SOURCE_DEFINITION_1 = f"{assets_path}/xl/pivotCache/pivotCacheDefinition1.xml"
SOURCE_RECORDS_1 = f"{assets_path}/xl/pivotCache/pivotCacheRecords1.xml"
SOURCE_DEFINITION_2 = f"{assets_path}/xl/pivotCache/pivotCacheDefinition2.xml"
SOURCE_RECORDS_2 = f"{assets_path}/xl/pivotCache/pivotCacheRecords2.xml"
...
"""
This function is designed to solve an pivot table cache issue.
Our table is populated until September 2020. Although, the "Total" value is brought
for October 2020. So we must switch all value from October 2020 to Total.
"""
...
if __name__=="__main__":
    if not os.path.exists(f"{datalake_path}/bronze"):
        os.mkdir(f"{datalake_path}/bronze")
    extract_oil_uf_product_df().to_parquet(f"{datalake_path}/bronze/oil_uf_product.parquet", index = False)
    extract_diesel_uf_type_df().to_parquet(f"{datalake_path}/bronze/diesel_uf_type.parquet", index = False)